...
> Last updated: April 2026 — added button hover state rules, brand color token clarification, updated Figma URL
...
These are the raw brand color values. They exist in the Figma `material/colors` collection. **All of these must be declared in the global CSS file** (`src/app.css`) so components can reference them. Reference semantic tokens (section 2.4) in components where they exist — use brand color tokens directly only where noted (e.g. button hover states).

```css
--ab-blue600: #1e9bd7;
--ab-blue700: #046ea0;   /* Client primary main */
--ab-blue800: #03567d;   /* Client primary dark — use for filled button hover */

--ab-purple600: #5949a7;
--ab-purple700: #4d3f91; /* Internal primary main */
--ab-purple800: #2f2759; /* Internal primary dark — use for filled button hover */
```

**Critical — these tokens exist and must be in the global CSS file.** If Claude Code reports these tokens do not exist, check `src/app.css` — they may be missing from the global declarations.
...
### 5.5 Button Hover State Rules

The `--primary_stateshover-*` opacity tokens are **not** correct for filled/contained button hover states. They are designed as overlays for light backgrounds only.

| Button variant | Hover mechanism | Token to use (Internal) | Token to use (Client) |
|---|---|---|---|
| Filled / Contained | Darken background color | `--ab-purple800` (#2f2759) | `--ab-blue800` (#03567d) |
| Outlined | Opacity overlay on light bg | `--primary_stateshover-light-internal` | `--primary_stateshover-light-client` |
| Text | Opacity overlay on light bg | `--primary_stateshover-light-internal` | `--primary_stateshover-light-client` |

**Never use the opacity hover token on a filled button.** The filled button hover must darken the background using the `--ab-purple800` or `--ab-blue800` brand color token.
...
- **File URL:** https://www.figma.com/design/k2AQYm5P4QC1fe0VEPxCew
...
16. **Do not use opacity hover token on filled buttons** — filled button hover must use `--ab-purple800` (Internal) or `--ab-blue800` (Client), never `--primary_stateshover-*`
17. **Do not declare that brand color tokens don't exist** — `--ab-purple600/700/800` and `--ab-blue600/700/800` are confirmed tokens; if missing from CSS, add them to `src/app.css`
